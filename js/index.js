...
fetch('https://api.edamam.com/search?q=pizza&app_id=80010e5d&app_key=a840721c6ce80a1b19aa39f1984cb906').then(resp => resp.json())
  .then(hits => {
    let output = '';
    hits.hits.forEach(hits => {
      ...
      <ul class='recipe' >
      <img onclick="openModal()" class="recipe-img" src='${hits.recipe.image}' />
      <li onclick="openModal()" class='recipe-title'>${hits.recipe.label}</li> <span>Health Labels:</span> 
